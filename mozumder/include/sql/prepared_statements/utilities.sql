...
AS
$BODY$
    SELECT coalesce(bot,FALSE) FROM mozumder_ip WHERE mozumder_ip.address = ip
$BODY$
LANGUAGE sql;
...
AS
$BODY$
    SELECT coalesce(bot,FALSE) FROM mozumder_useragent WHERE mozumder_useragent.user_agent_string = user_agent
$BODY$
LANGUAGE sql;
